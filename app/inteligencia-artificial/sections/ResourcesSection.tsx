...
const resources = [
  {
    title: 'Guía: IA para bioeconomía — por dónde empezar',
    type: 'PDF',
  ...
  {
    title: 'Benchmark: adopción de IA en bioeconomía LATAM',
    type: 'Informe',
      ...
      <div className="container-rl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {resources.map((resource, i) => (
            <SectionReveal key={resource.title} delay={i * 0.1}>
              <div className="border border-border-light p-6 rounded hover:border-rl-red/20 transition-colors">
                <span className="text-label-sm uppercase text-rl-red block mb-3">{resource.type}</span>
                <h3 className="font-display text-display-sm text-text-primary mb-3">{resource.title}</h3>
                <p className="text-body-sm text-text-secondary mb-6">{resource.description}</p>
                <Button variant="text" className="text-text-primary">
                  Descargar
                </Button>
              </div>
            </SectionReveal>
