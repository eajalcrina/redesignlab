    ...
    <section className="section-neutral">
      <div className="container-rl grid grid-cols-1 gap-10 py-24 lg:grid-cols-12 lg:gap-6">
        ...
        <ul className="border-t border-rl-dark lg:col-span-6 lg:col-start-7">
          {items.map((r, k) => (
            <li key={r.slug}>
              <button type="button" onClick={() => setOpen(r)} className="group grid w-full grid-cols-[56px_1fr] items-center gap-4 border-b border-border-light py-5 text-left md:grid-cols-[40px_64px_1fr_auto]">
                <span className="hidden font-mono text-[10.5px] tracking-[0.15em] text-text-tertiary md:inline">{String(k + 1).padStart(2, '0')}</span>
                ...
                <span>
                  ...
                  <span className="mt-2 block text-[13px] font-medium text-rl-red md:hidden">PDF ↓</span>
                </span>
                <span className="hidden text-[13px] font-medium text-rl-red md:inline">PDF ↓</span>
              </button>
            </li>
          ))}
        </ul>
